...
public class MessageController {
	...
	public ResponseEntity<java.util.List<MessageResponse>> batchPush(
		...
		logger.debug("Received batch push request for consumer group: {} with {} messages, scheduledAt={}",
	 ...
	 *
	 * Endpoint: POST /queue/batch/ack Headers: - consumerGroup: target consumer
	 * group (required) Body: - JSON array of message IDs to acknowledge Security:
	 * USER or ADMIN
	 *
	 * @param consumerGroup
	 *                      The consumer group header.
	...
	public ResponseEntity<Long> batchAck(@RequestHeader(CONSUMER_GROUP_HEADER) String consumerGroup,
			@RequestBody java.util.List<String> messageIds) {
		logger.debug("Received batch ack request for consumer group: {} with {} ids", consumerGroup,
				messageIds == null ? 0 : messageIds.size());
		long count = acknowledgementService.batchAck(consumerGroup, messageIds);
