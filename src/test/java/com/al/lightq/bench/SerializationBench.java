...
/**
 * Microbenchmarks for Message serialization/deserialization. Compares JSON
 * (Jackson) vs Smile+Afterburner (binary) used in Redis cache.
 *
 * Run: - From IDE: run the main(). - From CLI: mvn -q -DskipTests=false
 * -Dtest=com.al.lightq.bench.SerializationBenchTestPlaceholder test or run:
 * java -jar target/benchmarks.jar (when assembled with JMH plugin) or use
 * main().
 */
...
public class SerializationBench {
	...
	public static void main(String[] args) throws Exception {
		org.openjdk.jmh.runner.options.Options opt = new org.openjdk.jmh.runner.options.OptionsBuilder()
				.include(SerializationBench.class.getSimpleName()).detectJvmArgs().build();
		new org.openjdk.jmh.runner.Runner(opt).run();
	}
}

// Placeholder test name to allow mvn test to compile annotations without
// executing real benches.
// Not a JUnit test class; only used to trigger annotation processing when
// needed.
